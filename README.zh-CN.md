...
- 支持复用全局素材和断点续跑。
- 可直接从原生可编辑 PPTX 提取文字、形状、坐标和图片，重建阶段不再进行第二次图片模型调用。
- 内置 `editppt` 可编辑重建运行时。
...
重建时立即生成 native seed，最多进行两轮预览修正；仍未通过就写出明确失败原因，不再让页面无限运行。最终检查在 macOS 上优先使用 Quick Look，其他环境回退到 LibreOffice。
