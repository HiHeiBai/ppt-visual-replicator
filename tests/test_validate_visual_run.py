...
def write_valid_run(root: Path) -> tuple[Path, Path, Path]:
    ...
    for slide in source["slides"]:
        ...
        reference = f"references/reference-01/page-{number:03d}.png"
        ...
        pages.append({"target_slide": number})
        jobs.append(
            {
    ...
    (run / "visual-plan.json").write_text(
        json.dumps({"schema": "ppt_visual_plan.v1", "page_count": len(pages), "pages": pages}),
        encoding="utf-8",
    )
